...
def get_argparse():
    ...
    parser.add_argument('--output_dir', type=str, dest='output_dir',
                   default=datetime.now().strftime("results_%y%m%d_%p_%I:%M:%S"),
                   help='This is where the dumped pdbs will be stored')
    return parser
...
if Nmatch > 0:
    ...
    with open(args.output_dir+"/DHR_params.list", "w") as f:
        f.write("name repeat_length trans radius angle arc_length\n")
        for key, value in DHR_arcs.items():
            f.write(value+"\n")
...
#            p_index=params[0]
            ...
            new_pdb.dump_pdb('%s/%s_%s_%s_%s_%s.pdb'%(args.output_dir,DHR[0:base],n_sc_bb,torsions_string,angle,dist))
