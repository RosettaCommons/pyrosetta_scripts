...
#./sap_score_delta.py pdb1.pdb pdb2.pdb pdb3.pdb
# or
#./sap_score_delta.py -in:file:silent my.silent

# This program calculates the buried sap area between two proteins

# Your complex should be chainA binds to chainB
...
def get_sap_score(pose, subset):
    ...
    for resnum in range(1, pose.size()+1):
        ...
        for at in range(1, res.natoms()+1):
            ...
            if ( not subset[resnum] ):
                atom_sasa[-1] = 0

            skip_atoms.append(not subset[resnum])
            # if ( atom_sasa[-1] > 1000 ):
...
int_from_A = core.select.residue_selector.NeighborhoodResidueSelector(chainA, 16, False)
int_from_B = core.select.residue_selector.NeighborhoodResidueSelector(chainB, 16, False)

interface_sel = core.select.residue_selector.OrResidueSelector(int_from_A, int_from_B)
not_interface = core.select.residue_selector.NotResidueSelector(interface_sel)

delete_not_interface = protocols.grafting.simple_movers.DeleteRegionMover()
delete_not_interface.set_residue_selector(not_interface)

delete_B = protocols.grafting.simple_movers.DeleteRegionMover()
delete_B.set_residue_selector(chainB)

delete_A = protocols.grafting.simple_movers.DeleteRegionMover()
delete_A.set_residue_selector(chainA)
...
def sap_score(pose, name_no_suffix, out_score_map, out_string_map, suffix):
    ...
    delete_not_interface.apply(pose)
    # pose.dump_pdb("interface.pdb")

    complex_target_sap = get_sap_score( pose, chainB.apply( pose ) )
    complex_binder_sap = get_sap_score( pose, chainA.apply( pose ) )
    ...
    delete_B.apply(binder)
    delete_A.apply(target)

    # binder.dump_pdb("binder.pdb")
    # target.dump_pdb("target.pdb")

    sep_target_sap = get_sap_score( target, true_sel.apply( target ) )
    sep_binder_sap = get_sap_score( binder, true_sel.apply( binder ) )
    
    

    out_score_map['binder_delta_sap'] = sep_binder_sap - complex_binder_sap
    out_score_map['target_delta_sap'] = sep_target_sap - complex_target_sap

    print("Delta binder: %.1f"%out_score_map['binder_delta_sap'])
    print("Delta target: %.1f"%out_score_map['target_delta_sap'])
