...
class bb_hash:
    def __init__(self,resl,lever,hash_function,key_vars,store_vars):
        self.resl=resl
        self.lever=lever
    ...
    def get_bin_from_rot_and_bb(self,bb_rays,rot):
        ...
        Aatom=frame.global2local(bb_rays[0])
        Batom=frame.global2local(bb_rays[1])
        Catom=frame.global2local(bb_rays[2])
        Datom=frame.global2local(bb_rays[3])
        ABray = Ray(orig=Aatom, dirn=(Aatom-Batom))
        CDray = Ray(orig=Catom, dirn=(Catom-Datom))
        k=self.binner.get_key(ABray,CDray)
        return k
...
class bb_hash_6D:
    def __init__(self,cart_resl,ang_resl,target_angle):
        self.resl=cart_resl
        self.lever=ang_resl
        self.binner= RosettaStubHash(cart_resl,ang_resl)
        self.target_phi_psi = target_angle
...
class make_hash(bb_hash_6D):
    def __init__(self,resl,lever,target_angle,orient_atoms):
        bb_hash_6D.__init__(self,resl,lever,target_angle)
        self.n_added=0
        #generate rotamers of histidine in the helix
        self.rots = []
    ...
    def orient_rots(self,base,name):
        ...
        for angles in self.target_phi_psi:
            self.rots = self.rots + generate_canonical_rotamer_residues(name,angles)
